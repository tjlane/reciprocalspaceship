...
def _estimated_fft_memory(grid_shape: tuple[int, ...]) -> int:
    return int(np.prod(grid_shape, dtype=np.int64)) * FFT_BYTES_PER_GRID_POINT
...
def _materialized_correlation_grid(
    ...
    number_of_grid_points = int(np.prod(grid_shape, dtype=np.int64))
...
def has_origin_shift_ambiguity(spacegroup: SpaceGroupLike) -> bool:
    ...
    elif isinstance(spacegroup, int) and not isinstance(spacegroup, bool):
        if not 1 <= spacegroup <= 230:
            msg = (
                "spacegroup could not be converted to gemmi.SpaceGroup; "
                f"got {spacegroup!r}"
            )
            raise PhaseAlignmentInputError(msg)
...
def _periodic_local_maxima(correlation_grid: FloatArray) -> IntegerArray:
    ...
    descending_score_order = np.argsort(scores)[::-1]
...
def align_phases(
    ...
    *,
    ...
    maximum_refinement_starts : int, optional
        ...
        at least two so a competing solution cannot be hidden by the search bound.
    ...
    if best.correlation < validated_minimum_correlation:
        msg = (
            f"no clear origin-shift solution: best correlation {best.correlation:.3f} "
            f"is below {validated_minimum_correlation:.3f}"
        )
        raise NoClearSolutionError(msg)
    if (
        correlation_gap is not None
        and correlation_gap < validated_minimum_correlation_gap
    ):
        msg = (
            f"no clear origin-shift solution: correlation gap {correlation_gap:.3f} "
            f"is below {validated_minimum_correlation_gap:.3f}"
        )
        raise NoClearSolutionError(msg)
